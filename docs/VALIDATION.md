# Current validation: 0.2.2 · 12 September 2026

- Version 0.2.2 declares `httpx` as a runtime dependency and pins it plus `httpcore` in the production lock. It was previously present only in the development environment, so the RDP install succeeded but dashboard creation failed. Core verification now constructs the real dashboard in a temporary workspace, enters/exits its lifespan and exercises health, page, session and authenticated routes without model inference or external network access. Both Windows installers and the Mac installer run this check.
- A fresh Mac virtual environment containing only `requirements.lock` (no pytest or developer packages) passes dependency checks, the new startup probe and native CLI version check. A regression blocks `httpx` and confirms the new verifier fails; the old verifier incorrectly passed the same condition. Real Windows startup still needs the user's retry.
- The 0.2.1 Windows backup fix explicitly closes both SQLite connections before temporary-directory cleanup. Regression tests cover success, snapshot failure and refusal while Clara is actually running; both handle-lifetime cases failed before the fix. These tests simulate Windows handle-lifetime requirements on the Mac; the actual RDP retry remains to be confirmed. The updater now preserves the underlying error instead of attributing every backup failure to a running instance.
...
- The native Claude SDK account on this Mac is not signed in. **No fresh live-model inference is claimed for 0.2.2.** Earlier Windows live-model results remain historical evidence, not validation of every new feature.
