...
Clone the repository or extract Clara-Agent-0.2.2.zip into a new, persistent application folder belonging to the current user. A short path under LOCALAPPDATA is preferable to a deeply nested network folder. Keep the existing Clara application and Python folder in place.
