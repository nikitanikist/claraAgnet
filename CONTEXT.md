...
| --- | --- |
| Clara GitHub integration branch | `66962004943ed1a874cf9145aad96cd7042d8009` (16 Sep 2026); 429 tests pass, 4 skipped |
| Windows installed Clara | `ba4ed36` (installed by the user 16 Sep ~08:45 UTC: live desktop preview + ProFile support + skill). `23b228e` and `6696200` (restart exemption for general requests; Windows text-input helpers and the restarted service's console never count as leftovers) are **pushed, not installed**: the user installs them with `Update-ClaraPortalTest.ps1 -Revision 66962004943ed1a874cf9145aad96cd7042d8009` at idle. Worker capabilities now `["taxprep","profile"]` (set 08:47 UTC) `%LOCALAPPDATA%\Clara\portal.json` now has **`windows_handoff.qualified: true`** (set 06:38 UTC): automatic release after a verified closeout is on |
...
| Test frontend | v5.1 published (Lovable commits `94073dcf` "Clara workspace v5" and `25f40aba` "ProFile closeouts": live desktop card + viewer, delete with Undo, honest stage counter, mandatory Tax software field, optional instalments/t1135). Earlier: v4.1 published (Lovable commit `f8a61b4f`, 15 Sep 22:54 UTC): the Clara workspace redesign (fixed-height shell, Live indicator, grouped activity stream, stage chips, one-click answers, outputs-first details). Verified in Chrome after clearing a stale service worker; staff browsers may need a refresh. Lovable reports 4 pre-existing security-scan findings (Security view) that are not Clara's |
...
- `23b228e`, `6696200`: **restart and Windows-helper false positives.** The general-request observer skips the restarted worker's launch chain; `WINDOWS_HELPERS` (ctfmon, TabTip, conhost, OpenConsole, RuntimeBroker ...) are never leftovers in either observer. Found after a Softros test request (job `0fca16aa`) held the worker. Pushed, not installed.
- `32bf127`, `ba4ed36`: **ProFile (.25T) closeouts.** `SOFTWARE_PERMISSIONS` opens every gate to `software: profile` with `closeout.t1.profile`; `profile-fast-path` starter skill (window-title proof, conditional preparer update, Print/Email PDF, F12, F5 vs the 512,71 selector, instalments year+1, amounts trap, hard stops); optional `instalments`/`t1135` documents. Pushed, not installed; worker capabilities still `["taxprep"]`.
...
1. Install the latest revision (ProFile support + live desktop) on the server at idle (stop autostart and the python process, `.\Update-ClaraPortalTest.ps1 -Revision f60ba9e1184c26420c6723d6ebaec7aaf0f371f4`, `.\Set-ClaraAutostart.ps1 -Start`). Safe before or after the portal publish: the worker tolerates a portal without `clara-preview`, and the portal change is additive.
2. DONE 16 Sep 09:00 UTC: worker `6696200` installed and enrolled for `["taxprep","profile"]`; the portal Clara Settings page (v5.2, Lovable commit `f5e078f8`) offers TaxPrep and ProFile per runner and states that finished work returns to the assigner (the old `reviewer_email` setting row was deleted; `resolveReviewer` removed; clara-worker-key redeployed with the current contract). STILL TO CONFIRM on the server: the read roots live in `%LOCALAPPDATA%\Clara\settings.json` (`read_roots`, not portal.json; edited via the localhost dashboard Settings page or directly, re-read on every tool call) and must cover `O:\Clearhouse Clara agent\Profile files`; and Laureen's ProFile setup on the agent install. Lovable build "ProFile closeouts" (plan approved 16 Sep 08:45 UTC, message `umsg_01m2mntfm4fm9bbzhb9ska1wje`) adds the mandatory Tax software field, the assignment rules and the optional document types. First ProFile run: `Anderson, Patrick.25T` (single member, reference package exists) with the Live desktop open and a person watching; it is a real client return.
3. Lovable build "Clara workspace v5" (plan `.lovable/plan.md`, approved 16 Sep 07:53 UTC, message `umsg_01m2mk9w89eytbvwjm1tpkwnct`): migration (archive columns + RPCs, `clara_preview_frames`, `clara_preview_watch`, watch RPC), bucket `clara-previews`, functions `clara-preview` and `clara-preview-url`, frontend (indicator fix, delete with Undo, Live desktop card + viewer, freshness-aware "now" strip with 20/30/60 s rules, corrected stage keys and "Stage 4 of 7" counter, visual refresh), publish as v5.0. Verify in the browser after it lands; the plan page is https://claude.ai/artifact/YXfGJ2RrGS6rF2fzFGQzBN.
