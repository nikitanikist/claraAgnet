...
2. After the install, enrol ProFile: `select clara_set_worker_capabilities('2ce90208-c228-48f6-a457-63045dce50a0', '69af37fd-d2f6-4498-b871-6df8b15185db', '{"software":["taxprep","profile"],"general_chat":false}')` (or the Clara settings page), confirm the server's read roots include `O:\Clearhouse Clara agent\Profile files`, and confirm Laureen's ProFile setup on the agent install. Lovable build "ProFile closeouts" (plan approved 16 Sep 08:45 UTC, message `umsg_01m2mntfm4fm9bbzhb9ska1wje`) adds the mandatory Tax software field, the assignment rules and the optional document types. First ProFile run: `Anderson, Patrick.25T` (single member, reference package exists) with the Live desktop open and a person watching; it is a real client return.
