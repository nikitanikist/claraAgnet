...
def definitions(config,store,job):
    ...
    if attempt:
        ...
        items.append(('record_portal_delivery',
            'Record assigned T1 delivery from Chrome readbacks made in this task within the last 20 '
            'minutes. Each value may come from the named observation or any other fresh Chrome readback '
            '(navigate_page and list_pages show a page URL; take_snapshot shows names and emails). '
            'document_evidence_ids: the verified document evidence of every required PDF. pandadoc: one '
            'record per family member with member_id exactly as listed in the assignment, remote_id (the '
            'PandaDoc document ID from its /documents/ URL), url, external_key exactly the canonical '
            'reservation key closeout:<form>:pandadoc:<member_id>, and observation_id; readbacks must show '
            'that url, document ID, member name and member email. folder: remote_id (the OneDrive item '
            'ID), url, external_key exactly closeout:<form>:storage:folder, observation_id; readbacks must '
            'show the folder url and ID. files: one record per required PDF with member_id, document_type '
