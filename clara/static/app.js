...
function renderUsageSummary(){
    const reports=[...state.usageReports.values()].filter(row=>['completed','failed','cancelled','interrupted'].includes(row.status));
...
function updateUsageStatus(jobId,status){
    ...
    else if(['completed','failed','cancelled','interrupted'].includes(status))addUsage(jobId,{coverage:'unavailable',tokens:{},total_tokens:null,sdk_estimated_usd:null},status);
}
...
async function refreshStatus(){const s=await api('/api/status');state.status=s;const connected=s.auth.connected;const expired=s.auth.needs_login;const label=expired?'Claude login needs refresh':connected?`${s.auth.plan||'Claude'} sign-in found`:'Claude not connected';$('sidebar-connection').textContent=label;$('connection-dot').classList.toggle('online',connected&&!expired);$('auth-title').textContent=label;$('auth-description').textContent=s.auth.message;$('workspace-path').textContent=s.workspace;$('browser-status').textContent=s.connectors.browser.installed?'Connector installed · readiness checked when a task connects':'Connector missing · run the installer or npm ci';$('desktop-status').textContent=s.desktop.message;$('desktop-enabled').disabled=!s.connectors.desktop.supported_platform;$('notice').classList.toggle('hidden',connected&&!expired);$('notice').textContent=expired?'Claude’s stored session has expired. Run clara login in the terminal, then retry a task.':s.auth.message;}
async function loadSettings(){const s=await api('/api/settings');state.settings=s;$('model').value=s.model;$('max-turns').value=s.max_turns;$('timeout').value=s.task_timeout_minutes;$('budget-limit').value=s.max_budget_usd??'';$('read-roots').value=s.read_roots.join('\n');$('browser-enabled').checked=s.browser_enabled;$('desktop-enabled').checked=s.desktop_enabled;}
...
listen('settings-form','submit',async e=>{e.preventDefault();const data={model:$('model').value,max_turns:Number($('max-turns').value),task_timeout_minutes:Number($('timeout').value),max_budget_usd:$('budget-limit').value.trim()?Number($('budget-limit').value):null,read_roots:$('read-roots').value.split('\n').map(s=>s.trim()).filter(Boolean),browser_enabled:$('browser-enabled').checked,desktop_enabled:$('desktop-enabled').checked};await api('/api/settings',{method:'PUT',body:JSON.stringify(data)});await refreshStatus();toast('Settings saved for the next task.');});
