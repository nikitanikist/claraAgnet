...
async function loadSettings(){const s=await api('/api/settings');state.settings=s;$('default-mode').value=s.default_execution_mode;$('model').value=s.model;$('max-turns').value=s.max_turns??'';$('unlimited-turns').checked=s.max_turns===null;setTurnLimitControl('max-turns','unlimited-turns');$('timeout').value=s.task_timeout_minutes;$('budget-limit').value=s.max_budget_usd??'';$('read-roots').value=s.read_roots.join('\n');$('browser-enabled').checked=s.browser_enabled;$('desktop-enabled').checked=s.desktop_enabled;$('capture-evidence').checked=s.capture_evidence!==false;}
