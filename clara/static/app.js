...
async function loadSettings(){const s=await api('/api/settings');state.settings=s;$('model').value=s.model;$('max-turns').value=s.max_turns??'';$('unlimited-turns').checked=s.max_turns===null;setTurnLimitControl('max-turns','unlimited-turns');$('timeout').value=s.task_timeout_minutes;$('budget-limit').value=s.max_budget_usd??'';$('read-roots').value=s.read_roots.join('\n');$('browser-enabled').checked=s.browser_enabled;$('desktop-enabled').checked=s.desktop_enabled;$('capture-evidence').checked=s.capture_evidence!==false;}
...
listen('settings-form','submit',async e=>{e.preventDefault();const data={model:$('model').value,max_turns:$('unlimited-turns').checked?null:Number($('max-turns').value),task_timeout_minutes:Number($('timeout').value),max_budget_usd:$('budget-limit').value.trim()?Number($('budget-limit').value):null,read_roots:$('read-roots').value.split('\n').map(s=>s.trim()).filter(Boolean),browser_enabled:$('browser-enabled').checked,desktop_enabled:$('desktop-enabled').checked,capture_evidence:$('capture-evidence').checked};await api('/api/settings',{method:'PUT',body:JSON.stringify(data)});await refreshStatus();toast('Settings saved for the next task.');});
