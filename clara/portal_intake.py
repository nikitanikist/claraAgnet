...
def task_prompt(claim, messages, source_files=None):
    ...
    prompt = (
        ...
        'For a closeout, use the installed T1 skill for the assigned tax application (the assignment field '
        '"software") and the workflow already initialized for this exact closeout and family. '
        'For Personal Taxprep (software taxprep, a .125 file), load taxprep-fast-path: select exact LW/T183/ECL '
        'rows, use Ctrl+R/Ctrl+P, and print by form across members. For ProFile (software profile, a .25T '
        'file), load profile-fast-path: prove the member and form from the window title, update preparer '
        'information only when the audit Notice says so, print the return with File > Print/Email PDF and the '
        'T183 and engagement letter with F12, print any instalments or T1135 forms the return calls for, and '
        'never change Options or press Enter in Form Selection. '
        'For the OneDrive filing step load onedrive-filing: find the existing client folder by searching one '
        'name at a time before ever creating one, upload the package into this year folder inside it, and '
        'share that folder view-only using Copy link. The blue Send button in that dialog emails the '
        'client and is never pressed. '
        ...
        'sign, e-file, or mark the closeout finally completed. T2 and T3 execution are unavailable. '
        ...
        'Programs already open before this task belong to whoever opened them: leave them, unless one blocks '
        'your work, in which case close it with its normal close control; if closing asks whether to save changes '
        'that are not yours, cancel, leave it open and ask the staff member. Before finishing, close every window '
        'you opened for this task.\n\n'
