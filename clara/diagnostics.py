"""Bounded local tool diagnostics; never store image bytes or execute a recovery."""
import json
import re
...
def tool_diagnostic(data, duration_ms=None):
    ...
    return {"failed": bool(data.get("hook_event_name") == "PostToolUseFailure" or structured_error or connector_error),
            ...
            "note": "Tool return evidence only; task completion requires verification. Images are not retained."}
