...
**The drop point becomes the field's top-left corner, not its centre.** Dropping on the printed line hangs the whole field below it, and a signature renders near the bottom of its box, so it lands under the line. Aim the drop roughly one field-height ABOVE the line so the field's bottom edge rests on the line. This was measured on the first real TaxPrep package, where the T183 field sat about 50 px low and the engagement letter about 35 px low: close enough to look right, wrong where it matters. After each placement, confirm the line meets the field at or near its bottom edge and that the box does not cover the caption beneath the line. The date field follows the same rule.
