...
**The drop point becomes the field's top-left corner, not its centre.** Dropping on the printed line hangs the whole field below it, and a signature renders near the bottom of its box, so it lands under the line. Aim the drop roughly one field-height ABOVE the line so the field's bottom edge rests on the line. This was measured on the first real TaxPrep package, where the T183 field sat about 50 px low and the engagement letter about 35 px low: close enough to look right, wrong where it matters.

**Place each field by looking, not by aiming once.** The reviewer's standard is exactly on the line, not one step above or below. Drag the field roughly into place, then zoom into just that field and its line, read the gap between the **bottom edge of the field** and the line, and move the field by the gap you actually see. Re-zoom after each move and judge from the new view. Stop when the bottom edge meets the line, within a pixel or two at the magnification you are looking at, and no part of the box covers the caption beneath it. Before leaving the field, confirm it belongs to the member it is meant for; the editor shows the assignee on the selected field, and a perfectly placed signature assigned to the wrong member is still wrong. Judge each correction by eye from the view in front of you rather than by arithmetic on the image: each pass lands closer than the last. Move the field that is already there rather than dropping a second one; prefer an arrow-key nudge for the final correction if the editor supports it, confirming from the zoom that it moved; and if four corrections have not settled it, stop and ask. Keep the final zoomed view of each field as evidence. The date field follows the same loop against its own line. This loop is the deliberate exception to spending turns sparingly: a handful of extra observations here costs under a minute and decides whether the packet is accepted.
