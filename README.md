...
**Version 0.2.4 restores continuation after a stopped task with incomplete usage** through Workflow review, while keeping the unknown usage, saved session and checkpoints. Autonomous is now the default task mode, with a saved Ask option and individual overrides. [Recovery instructions](START-HERE.md#resume-after-stopping-a-task).
